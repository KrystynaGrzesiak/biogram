...
test_features <- function(target, features, criterion = "ig", adjust = "BH",
  ...
  features_names <- names(features)

  feature_size <- if ("simple_triplet_matrix" %in% class(features)) {
    slam::col_sums(features)
  } else if ("dgCMatrix" %in% class(features)) {
    features_names <- attr(features, "Dimnames")[[2]]
    Matrix::colSums(features)
  } else {
    colSums(features)
  }

  informative_features <- feature_size > threshold & feature_size < (nrow(features) - threshold)

  # eliminate non-informative features
  features <- features[, informative_features, drop = FALSE]
  features_names <- features_names[informative_features]
  p_vals <- if(quick) {
